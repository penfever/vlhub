...
#/scratch/bf996/ArborCLIP_Benchmarking/metadata/confounding_species.csv

try:
    confounding_path = Path("./metadata/confounding_species.csv")
    confounding_df = pd.read_csv(confounding_path)
except:
    confounding_path = Path("vlhub/metadata/confounding_species.csv")
    confounding_df = pd.read_csv(confounding_path)

def get_confounding_idx(cn):
    classnames = confounding_df[confounding_df["scientific_name"].isin(cn)]["scientific_name"].unique()
    indices = [confounding_df[confounding_df["scientific_name"] == c]['idx'].unique().item() for c in classnames]
    return np.array(indices)

def get_confounding_pairs():
    return [
            ["Harmonia axyridis", "Adalia bipunctata"],
            ["Harmonia axyridies", "Epilachna mexicana"],
            ["Phyllopertha horticola", "Popillia japonica"],
            ["Euschistus servus", "Halyomorpha halys"],
            ["Euschistus tristigmus", "Erthesina fullo"],
        ]